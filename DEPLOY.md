# 🚀 Деплой Luverano (VPS/Timeweb Cloud)
...
1. Создайте приложение типа Node.js и подключите репозиторий GitHub.
2. Переменные окружения: перенесите из `.env.local` в UI (секция Variables).
3. Команды:
   - Build: `npm ci && npm run build`
   - Start: `node .next/standalone/server.js`
4. PORT не задавайте вручную — платформа передаёт его в `process.env.PORT`.
5. Включите автодеплой “On Push”.

Оптимизация изображений: включена в `next.config.ts` (sharp рекомендуется).
