...
const TopCollections = ({data}:{data:ProductType[]}) => {
    ...
    return (
        <section className='bg-home-bg-1 lg:pt-25 pt-8 lg:pb-25 pb-15 group/section'>
            <div className='container md:flex justify-between md:items-center gap-4 mb-10' ref={containerRef}>
                <Title>Популярные коллекции</Title>
                <Link href={"/catalog"} className='text-gray-1-foreground lg:text-xl text-lg border-b border-b-primary mt-2.5 md:mt-0 inline-block hover:border-b-primary hover:text-secondary-foreground duration-500'>Смотреть все коллекции</Link>
            </div>
                    ...
                    {data.map((prd) => {
                        return (
                            <SwiperSlide key={prd.id}>
                                <Card key={prd.id}>
                                    <CardHeader>
                                        <CardImg src={prd.thumbnail} height={400} width={340} href={`/product-details?id=${encodeURIComponent(String(prd.id))}`} />
                                    ...
                                    <CardFooter>
                                        <CardTitle path={`/product-details?id=${encodeURIComponent(String(prd.id))}`}>{prd.title}</CardTitle>
