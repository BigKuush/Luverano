...
const ProductQuickView = ({ isDialogOpen, setIsDialogOpen, product }: ProductQuickViewType) => {
    return (
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent showCloseButton={false} className='max-w-[1230px] p-0 sm:rounded-none rounded-none border-0 overflow-y-auto [&_.close-orginal]:hidden overflow-visible'>
                ...
                <div className='max-h-[667px] h-full flex md:flex-row flex-col items-start gap-10 p-10 overflow-y-auto scrollbar-hidden'>
                    <div className='max-w-[560px] w-full relative bg-[#F2F2F2]'>
                        <Image
                            ...
                            alt='img'
                        />
                        <Link href={`/product-details?id=${encodeURIComponent(String(product.id))}`} className='block w-full bg-primary text-white text-center text-xl font-medium leading-[150%] py-[15px] px-7.5 absolute bottom-0 left-0 cursor-pointer'>Смотреть товар</Link>
                    </div>
